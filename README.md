...
### Motion event — end-to-end sequence

Full round-trip triggered by a single PIR pulse. MQTT carries only the
*notification* and the *ack*; the JPEG travels over HTTP but **through
the backend proxy**, never on a direct ESP32↔browser connection.

```mermaid
sequenceDiagram
    autonumber
    participant PIR as HC-SR501
    participant ESP as ESP32-CAM<br/>firmware.ino
    participant MQ as Mosquitto
    participant API as FastAPI<br/>(MQTT client +<br/>HTTP proxy)
    participant DB as SQLite
    participant UI as Dashboard<br/>(local or via ngrok)

    PIR->>ESP: GPIO 13 HIGH (ISR, rising edge)
    Note over ESP: motionFlag = true

    ESP->>MQ: PUBLISH security/alert<br/>{event:"motion", device_id, uptime, rssi}
    MQ->>API: deliver (aiomqtt subscriber)
    par persist
        API->>DB: INSERT INTO events
    and fan out
        API-->>UI: WS push<br/>{topic:"security/alert", payload:{...}}
    end
    Note over UI: red flash on #alert-panel<br/>auto-fetch image

    rect rgb(254, 242, 220)
    Note over UI,ESP: HTTP image path -- always via backend proxy
    UI->>API: GET /api/capture
    API->>ESP: GET http://<esp-lan-ip>/capture<br/>(LAN-only, never exposed)
    ESP->>ESP: esp_camera_fb_get()
    ESP-->>API: 200 image/jpeg<br/>(SVGA JPEG, ~40–80 KB)
    API-->>UI: 200 image/jpeg<br/>Cache-Control: no-store
    Note over UI: render into #cam-img
    end

    UI->>API: POST /api/ack
    API->>MQ: PUBLISH security/ack<br/>{ack:"image_received"}
    MQ->>ESP: deliver (subscribed)
    Note over ESP: awaitingAck = false<br/>(Sleep Guard released)

    opt ALWAYS_ON = 0
        ESP->>MQ: PUBLISH security/status retained<br/>{status:"sleeping"}
        Note over ESP: esp_deep_sleep_start()<br/>wake source: GPIO 13 HIGH (ext0)
    end
```

### Firmware state machine — Sleep Guard

In `ALWAYS_ON=0` mode the firmware cycles through these states. The Sleep
Guard is the 30 s window in the `Alerted` state — the device only powers
down once it has confirmation that the dashboard actually received the
image, or the fallback timeout elapses (so an unreachable dashboard can't
pin the device awake forever).

```mermaid
stateDiagram-v2
    [*] --> Boot: power on<br/>or ext0 wake
    Boot --> Online: init cam → Wi-Fi →<br/>MQTT → HTTP server
    Online --> Alerted: PIR ISR fires<br/>OR security/command {capture}
    Alerted --> Alerted: heartbeat /<br/>LED commands
    Alerted --> DeepSleep: security/ack received<br/>(awaitingAck = false)
    Alerted --> DeepSleep: 30 s window expired<br/>(fallback timeout)
    DeepSleep --> [*]: publish retained<br/>status="sleeping",<br/>then ESP off
    [*] --> Boot: PIR HIGH on GPIO 13<br/>wakes chip via ext0

    note right of Boot
        bootCount++ (RTC memory)
        wake_reason decides
        whether boot is a
        motion event
    end note

    note left of Alerted
        publish security/alert
        awaitingAck = true
        activeWindowEnd = now + 30 s
        HTTP /capture stays up
    end note
